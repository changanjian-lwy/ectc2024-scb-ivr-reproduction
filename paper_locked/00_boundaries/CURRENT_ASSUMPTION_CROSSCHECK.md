...
| Flying-capacitor ripple | Uses steady charge/volt-second balance; no complete numeric capacitor model | Initially assumes C1/C2 ripple is zero for mode derivation, then includes ripple in design considerations | Finite 53.8 uF candidate capacitors, so ripple is allowed | Value is cross-source candidate, not a P24 value -- see the "Flying capacitor" row below for the 2026-09-15 cross-topology provenance finding this value carries |
| Output capacitor value (`Cout`) | Conceptual/ripple discussion; no complete populated value | Gives capacitor family but incomplete suffix/value | `4.672 mF` cross-source candidate | **Same provenance issue as `Cfly` below, checked 2026-09-15**: `4.672 mF` is the literal sum of EPE2019 Table I's output-capacitor bank (`8x220uF+32x47uF+64x22uF=4672uF`) -- i.e. also sourced from that paper's CSC-buck prototype parts list, not a value for the conventional SC buck P24 actually is. Used in the R03A/R03D/R04E3 family (`Vout` bootstrap attempts); not used by Track A's local-commutation chain. |
