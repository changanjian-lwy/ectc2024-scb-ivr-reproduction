"""Generate A47 stage-2's final 9.60%-9.70% local refinement grid (0.01% steps).

Mirrors build_a42_threshold_fine_refinement.py's / build_a45_stage2_fine_
refinement.py's 1-basis-point mechanism, inside the bracket
build_a47_stage2_refinement.py's 0.1%-step grid found (9.6%: no ZVS, min
Vds=36.9 mV; 9.7%: ZVS, min Vds=-8.4 mV).
"""
...
OUT = HERE / "fine_cases"
NEGATIVE_BASIS_POINTS = tuple(range(960, 971))
...
def build() -> list[Path]:
    ...
    for basis_points in NEGATIVE_BASIS_POINTS:
        ...
        text = text.replace(
            ...
            f"* A47 stage 2 fine refinement - nonlinear Coss(V) {pct:.2f}% negative-current row (P25_SUPPLEMENT/SENSITIVITY_ONLY)",
            1,
        )
        text = text.replace(
        ...
        path = OUT / f"a47_fine_{basis_points:04d}bp_nonlinear.cir"
