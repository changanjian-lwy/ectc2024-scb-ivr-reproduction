...
(byte-for-byte identical logic) from R04E24's own committed
`scripts/ltspice_raw_parser.py` (itself unchanged since R04E11-R04E23),
reused here rather than re-derived, per this project's own standing
convention of not re-inventing an already-verified raw-format parser.
See those scripts' own docstring for the original format-determination
notes (LTspice 26.0.2 for MacOS, UTF-16LE text header terminated by
"Binary:\\n", real/forward/nocompression flags, time as 8-byte double +
remaining vars as 4-byte singles per point).
