"""Analyze R04E7's voltage-ratio-gated EPE2019 3-state charge-redistribution
ladder-bootstrap grid.

Parses each generated case's LTspice `.log` for its `.meas` results (same
convention as `analyze_b02...`), determines the actual number of a-b-c
cycles completed (the largest `vc1_cycN` that is present in the log --
`.meas` lines whose `WHEN ... FALL=N` event never occurred are simply
absent from the log, matching how missing per-cycle measurements are
already handled in `analyze_b02`), computes `LADDER_ERR` at every completed
cycle checkpoint and at the run's own terminal state, and reports whether
the run converged (`STATE_FINAL==3`, i.e. reached the `DONE` state) or hit
the safety cap without converging (`STATE_FINAL==4`, `CAPPED`).

`LADDER_ERR` is the SAME metric R02B/R04E6 already use:
...
NAME_RE = re.compile(r"r04e7_tol_(?P<tol>[0-9p]+)pct")

MAX_CYC_SCAN = 60  # scan a bit beyond the 50 generated checkpoints; any
# genuinely absent measurement is just skipped, this only bounds the regex
# search, not the actual convergence result.
...
# t_terminal is a WHEN-type .meas ("t_terminal: V(state_mon)=2.5  AT
# <time>"), so its value sits after "AT", not after the first "=" (which
# would otherwise wrongly capture the 2.5 threshold) -- parsed separately.
...
def parse_case(cir: Path) -> dict:
    ...
    tol = float(match.group("tol").replace("p", ".")) / 100.0
...
def main() -> None:
    ...
    rows.sort(key=lambda r: r["tol_pct"])
