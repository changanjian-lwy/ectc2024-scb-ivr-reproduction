...
| R04E16 | Roberts' PhD dissertation Sec. 3.5 soft-start mechanism (Vin ramped slowly through an eFuse while P24's ORDINARY fixed-timing four-phase PWM, R03A's own gate formulas, runs unchanged from t=0) -- genuinely different from R04E5's already-falsified "ramp grafted onto the strict event-gated controller" combination; base netlist copied from R03A with TSTART=0, LPHASE=1.4666667nH (was R03A's 2.68nH), CFLY swept, and R02B's passive-divider precharge network removed entirely (true zero-energy start, no auxiliary precharge circuit); sweep Cfly in {0.6,3,8.7}uF x Tramp at Roberts' own 30x margin (3 cells), plus Cfly=3uF x Tramp at 10x/100x margin (2 cells), plus a Cfly=3uF/Tramp=1us control cell (near-instant ramp) -- 6 cells total, TSTOP=Tramp+300us each | Pilot confirmed TSTART=0 gating works correctly (correct Ton/T/phase-spacing, one negligible ~5e-17s floating-point boundary artifact at t=0 reported transparently); EVERY cell first attempted at R03A's own default solver options hit a reproducible, forensically-confirmed solver breakdown (an independent PWL source reading physically impossible values after classic retry-chatter, at varying times unrelated to actual circuit stress) -- fixed by adopting solver=alt/cshunt=1e-15/plotwinsize=0, the exact convention R04E5-R04E10 already use at this TMAX=50ps; 4 of 6 cells (control, Cfly=3uF at 10x and 30x margin, and Cfly=0.6uF at 30x margin -- the last one added in a same-day follow-up commit after it finished running unattended overnight, 11842.4s wall time) reached completion before a second, independent tooling finding -- severe, TSTOP-disproportionate runtime variance (1302.6/1932.1/7937.0/11842.4s wall-clock for similar-magnitude TSTOP values that do not scale monotonically with TSTOP) -- made completing the remaining 2 (Cfly=8.7uF, 100x margin) impractical within one session; ALL 4 completed cells avoid R03A's own catastrophic runaway (max phase current 86.9-150.1A, at least 5.9x below R03A's own smallest peak, all within the +/-250A bound) with NO Vout overshoot -- but the control cell (near-instant ramp) equally avoids it, directly undermining a clean "the ramp specifically is responsible" causal claim; most likely explanation is that removing R02B's passive-divider precharge network (which is what let R03A's ladder pre-charge to 34/22/11V before a cold-Vout PWM hit it) eliminates R03A's own diagnosed mismatch independent of ramp speed; the three Cfly=3uF cells converge to nearly identical Vout~0.559V/LADDER_ERR~1.34-1.35 despite a 69x Tramp range, while peak IL1 varies non-monotonically with Tramp (150.1/111.6/86.9A); the single Cfly=0.6uF point (Vout=0.564V, LADDER_ERR=1.399, IL1_max=92.2A) is modestly but not dramatically different, too few points to establish a Cfly trend |
...
## R04E16 -- extending Roberts' own dissertation soft-start method into
   ...
   counterfactual shows the ramp itself is probably not why
...
`TMAX=50 ps` resolution -- `TMAX` itself was never coarsened.
...
runtime variance (`1302.6/1932.1/7937.0 s` of real LTspice compute time
for three cells with very similar `TSTOP`), and running cells
concurrently made this roughly `20x` worse (this machine's `10` physical
cores get oversubscribed by each LTspice process's own `10`-thread
request). Only `3` of the `6` planned cells (the control cell and both
`Cfly=3 uF` cells at `10x`/`30x` margin) could be completed within a
practical session budget; the remaining `3` (`Cfly=0.6`/`8.7 uF`, and
the `100x`-margin cell) were left incomplete, reported honestly rather
than silently patched over by coarsening `TMAX`.

**The 3 completed cells all avoid R03A's own catastrophic runaway**
(max phase current `86.9-150.1 A`, at least `5.9x` below R03A's own
smallest peak of `563 A`, comfortably inside the `+/-250 A` safety
bound) with no `Vout` overshoot at all (the opposite of R03A's own
`54%` overshoot). **But the control cell (`Tramp=1 us`, an
...
a clean "the `Vin` ramp specifically is responsible" causal claim. All
three completed cells converge to nearly identical final state
(`Vout~=0.559 V`, `LADDER_ERR~=1.34-1.35`) despite a `69x` range in
`Tramp`, strongly suggesting the dominant effect is removing R02B's
passive-divider precharge network itself (which is what let R03A's own
ladder pre-charge to `34/22/11 V` before full-strength PWM hit a still
-cold `Vout` -- R03A's own diagnosed failure mechanism), not the ramp
speed. Peak inductor current does vary meaningfully but
non-monotonically with `Tramp` (`150.1/111.6/86.9 A` at
`Tramp=1/22.87/68.61 us`). See
`R04E16_roberts_softstart_fixed_timing/BOUNDARY.md` and `RESULTS.md` for
the full forensic evidence, the 3-cell results table, and the explicit
discussion of what remains untested.
