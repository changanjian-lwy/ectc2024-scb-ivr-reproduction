"""Analyze R04E15's fine-grid refinement of R04E14's passive-precharge
PASS cells.

Parses each generated case's LTspice `.log` for its non-stepped `.meas`
results (identical single-cell log format to R04E14's own
`analyze_r04e14.py`), computes the same `LADDER_ERR` metric R02A/B/R04E14
already use, and writes results.csv/results.json for ALL committed cases
(Grid A + Grid B + Grid C once built).

Case filenames are `e15_c<CDIV>_t<TRAMP>_f<CFLY>` (values in uF/us, 'p'
for a decimal point) -- kept short per build_r04e15.py's own NAMING
section, reusing R04E14's convention verbatim.
"""
...
NAME_RE = re.compile(r"e15_c(?P<cdiv>[0-9pP]+)_t(?P<tramp>[0-9pP]+)_f(?P<cfly>[0-9pP]+)")
...
def parse_case(cir: Path) -> dict:
    ...
    if "unable to open database file" in log and not re.search(r"^vc1_final:", log, re.MULTILINE):
        raise RuntimeError(
            ...
            "see R04E14's build_r04e14.py NAMING NOTE). This case must be "
            "re-run."
        )
...
def main() -> None:
    rows = [parse_case(p) for p in sorted(CASES.glob("e15_*.cir"))]
